...
import {
  DescriptionableUnion,
  DescriptionObjectType,
  DescriptionType,
} from './description.dto';
import {
  GraphqlLoader,
  Loader,
...
export class DescriptionResolver {
  constructor(
    @InjectRepository(DescriptionText)
    public readonly descriptionTextRepository: Repository<DescriptionText>,
    @InjectRepository(DescriptionChecklist)
    public readonly descriptionChecklistRepository: Repository<DescriptionChecklist>,
  ) {}
    ...
    for (const item of loader.polimorphicTypes) {
      switch (item.descriminator) {
        case DescriptionType.Text:
          const textDescriptions = await this.descriptionTextRepository
            .createQueryBuilder()
          ...
          results.push({
            descriminator: DescriptionType.Text,
            entities: textDescriptions,
          });

          break;
        case DescriptionType.Checklist:
          const checklistDescriptions =
            await this.descriptionChecklistRepository
              .createQueryBuilder()
          ...
          results.push({
            descriminator: DescriptionType.Checklist,
            entities: checklistDescriptions,
          });
