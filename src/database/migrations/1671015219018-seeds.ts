  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    for (const i of userSec) {
      ...
      for (let t = 1; t <= numberOfTasksForEachUser; t++) {
        ...
        for (let d = 1; d < 3; d++) {
          const description_text = await queryRunner.connection
            .getRepository(DescriptionText)
            .save({
          ...
          const description_checklist = await queryRunner.connection
            .getRepository(DescriptionChecklist)
            .save({
          ...
          for (let ci = 0; ci < 3; ci++) {
            await queryRunner.connection
              .getRepository(DescriptionChecklistItem)
              .save({
